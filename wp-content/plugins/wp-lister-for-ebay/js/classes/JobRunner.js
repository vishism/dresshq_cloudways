...
WpLister.JobRunner = function () {
    ...
    var runJob = function ( jobname, title, extra_params ) {
        ...
        var params = {
            ...
            job: jobname,
            _wpnonce: wpl_JobRunner_i18n.wple_ajax_nonce
        };
        ...
        .done( function( response ) {
            ...
            if ( jQuery.isArray(self.jobsQueue) && ( self.jobsQueue.length > 0 ) ) {
                ...
                var multithread_jobs = ['reviseItems', 'updateItems', 'verifyItems', 'reviseAllChangedItems', 'verifyAllPreparedItems', 'updateAllPublishedItems', 'runImport', 'endItems'];
        ...
        .fail( function(e,xhr,error) {
            ...
            if ( ( e.status == 403 ) || ( e.status == 404 ) || ( e.status == 500 ) || ( e.status == 0 ) ) {
                ...
                if ( ( wplister_ajax_error_handling == 'retry') && ( self.retryCount < 5 ) ) {
                    ...
                    logMsg += '<b>Error: Failed to process this task. Proceeding with next task...</b><br><small>Debug info: The server responded with HTTP code ' + e.status + ' and returned:</small><pre>' + jQuery("body").text(e.responseText).html() + '</pre>';
                    logMsg += '</div>';
                    jQuery('#wple_jobs_log').append( logMsg );
