...
{ 
	...
	// public    $_transportOptions = array(); // made public for ListingsModel::sendMessageXmlStyle() (OBSOLETE since 2.2.4)
	private $_callAttempts = 0;
    // ***** END EBATNS PATCH *****
	...
	protected function _startTp( $key )
	{
		...
		if (empty($this->_loggingOptions['LOG_TIMEPOINTS']))
			return;
		// ***** END EBATNS PATCH *****
	...
	protected function _incrementApiUsage($apiCall)
	{
		...
		if (empty($this->_loggingOptions['LOG_API_USAGE']))
			return;
		// ***** END EBATNS PATCH *****
	...
	function call( $method, $request, $parseMode = EBATNS_PARSEMODE_CALL )
	{
		...
		// ***** BEGIN EBATNS PATCH *****
		// Always check for oAuth token expiry
		$new_token = WPLE_eBayAccount::maybeMintToken( $this->_session->wple_account_id, false );

		if ( $new_token ) {
			$this->_session->setRequestToken( $new_token );
		}
		// ***** END EBATNS PATCH *****
		...
		// ***** BEGIN EBATNS PATCH *****
		// Expired OAuth token
		if ( is_a( $ret, 'EbatNs_ResponseError' ) ) {
			foreach ( $ret->Errors as $error ) {
				if ( $error->ErrorCode == '21917053' || $error->ErrorCode == '930' ) {
					if ( $this->_callAttempts < 2 ) {
						//WPLE_eBayAccount::maybeMintToken( $this->_session->wple_account_id, true );
						$new_token = WPLE_eBayAccount::maybeMintToken( $this->_session->wple_account_id, true );
						...
						// retry this call
						WPLE()->logger->info( 'Retrying call' );
						$ret = $this->call( $method, $request, $parseMode );
					} else {
						// allow $ret to be returned but reset the attempts back to 0
						//$this->_callAttempts = 0;
					}
				} else {
					//$this->_callAttempts = 0;
				}
	...
	function sendMessage( $message )
	{
		...
		// ***** BEGIN EBATNS PATCH *****
		// Set OAuth to TokenMode 2 to prevent this library from adding the RequestCredentials element to the requests
		//if ( $this->_session->getTokenMode() == 0 && $this->_session->getAuthType() == EBAY_AUTHTYPE_OAUTH )
		if ( $this->_session->getTokenMode() == 2 && $this->_session->getAuthType() == EBAY_AUTHTYPE_OAUTH )
		{
		...
		curl_setopt( $ch, CURLOPT_USERAGENT, 'ebatns/1.1' . ';wple/' . WPLE_PLUGIN_VERSION . ';wc/' . WC_VERSION . ';wp/' . get_bloginfo( 'version' ) . ';php/' . phpversion() );
		...
		// ***** BEGIN EBATNS PATCH *****
		if ( isset($this->_transportOptions['HTTP_VERBOSE']) && $this->_transportOptions['HTTP_VERBOSE'] )
		{
	...
	function sendMessageShoppingApiStyle( $message, $extraXmlHeaders )
	{
        ...
        // ***** BEGIN EBATNS PATCH *****
		// curl_setopt( $ch, CURLOPT_HTTP_VERSION, 1 ); // disabled as it can lead to "curl_error 56 OpenSSL SSL_read: Success"
        // regard WP proxy server
        if ( defined('WP_USEPROXY') && WP_USEPROXY ) {
            if ( defined('WP_PROXY_HOST') && defined('WP_PROXY_PORT') )
				curl_setopt( $ch, CURLOPT_PROXY, WP_PROXY_HOST . ':' . WP_PROXY_PORT );
    ...
    // ***** BEGIN EBATNS PATCH *****
	// allow to set endpoint to call sendMessageXmlStyle() from other classes
	function setEP( $endpoint )
	{
		$this->_ep = $endpoint;
	}
    // ***** END EBATNS PATCH *****	
