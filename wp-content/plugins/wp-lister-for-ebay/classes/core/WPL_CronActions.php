...
class WPL_CronActions extends WPL_Core {
	...
	public function __construct() {
        ...
        add_action( 'init',                                         'wple_action_scheduler_settings', 50 );
        add_action( 'wple_do_background_revise_items',              'wple_do_background_revise_items', 10, 5 ); // this could go into another class...
        add_action( 'wple_retry_handle_reduce_order_stock',         array( 'OrderWrapper', 'handle_reduce_order_stock' ) );
        add_action( 'wple_bg_complete_sale_on_ebay',                array( 'WpLister_Order_MetaBox', 'callCompleteOrder'), 10, 2 );
	...
	public function cron_update_auctions() {
		...
		if ( ! empty( $accounts) ) {
			...
			foreach ( $accounts as $account ) {
                ...
                $this->EC->updateActiveOrders();

				$this->EC->updateListings(); // TODO: specify account
				$this->EC->updateEbayMessages();

				do_action( 'wple_update_auctions_for_account', $account );
        ...
        // When an external cron is being used, trigger action-scheduler so it revises listings in the background #50042
        if ( get_option( 'wplister_cron_auctions' ) == 'external' ) {
	...
	public function action_clean_log_table() {
		...
		if ( $days_to_keep = get_option( 'wplister_log_days_limit', 30 ) ) {
           ...
           WPLE()->logger->delete_old_logs( $days_to_keep );
		}
    ...
    public function set_inventory_check_cron_schedule() {
        ...
        if ( get_option( 'wplister_run_background_inventory_check', 1) ) {
            // Turn it on
            ...
            if ( as_next_scheduled_action( 'wple_bg_inventory_check_run_tasks' ) ) {
                as_unschedule_all_actions( 'wple_bg_inventory_check_run_tasks' );
            }
        }
    }
    ...
    public function cron_bg_inventory_run_tasks() {
        ...
        if (! get_option( 'wplister_run_background_inventory_check', 0 ) ) {
            WPLE()->logger->info( 'Background inventory check is disabled. Skipping');
            return;
        }
