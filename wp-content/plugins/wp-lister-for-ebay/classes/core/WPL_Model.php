...
class WPL_Model {
	...
	function handleResponse( $res, $is_second_request = false )	{
		...
		if ( ! is_object($res) ) {
            ...
            $errorObj->SeverityCode = 'Error';
            $errorObj->ErrorCode 	= 500;
            $errorObj->ShortMessage = 'eBay response is invalid';
            $errorObj->LongMessage 	= 'eBay response is invalid. Please see the logs for more details';
            $errorObj->HtmlMessage 	= 'eBay response is invalid. Please see the logs for more details';
            WPLE()->logger->error('eBay response is not an object: '.print_r($res,1));

            $this->result = new stdClass();
            $this->result->success = array();
		...
		foreach ($res->getErrors() as $error) {
			...
			if ( $error->getErrorCode() == 931 ) { 
				...
				$longMessage .= '<br>'. 'To refresh your eBay API token, please visit Settings &raquo; Account &raquo; Edit and follow the instructions in the sidebar.';
			...
			// #17470 - Your security token has expired
			if ( $error->getErrorCode() == 17470 ) { 
				$shortMessage = 'Your API token has expired';
				$longMessage .= '<br><br>'. '<b>Please authenticate WP-Lister with eBay again.</b>';
				$longMessage .= '<br>'. 'This can happen if your token was revoked or has expired.';
				$longMessage .= '<br>'. 'To refresh your eBay API token, please visit Settings &raquo; Account &raquo; Edit and follow the instructions in the sidebar.';
				update_option( 'wplister_ebay_token_is_invalid', array( 'site_id' => $this->site_id, 'account_id' => $this->account_id ) ); // store site and account
			}
		...
		if ( $res->getMessage() ) { 
			...
			$errorObj->ShortMessage = __( 'Additional details about this error', 'wp-lister-for-ebay' );
	...
	function save_last_result() {
		...
		update_option( 'wplister_last_product_update_results', $update_results, false );
