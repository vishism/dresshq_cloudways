...
if ( ! class_exists( 'WPL_StocksLogger' ) ) :

    class WPL_StocksLogger {

        public function __construct() {
            // Allow plugins to turn stock logging off #47301
            if ( apply_filters( 'wple_enable_stock_logging', true ) ) {
        ...
        /**
         * Delete all stock log entries and optimize the logs table as well
         */
        ...
        public static function optimizeLog() {
            global $wpdb;
            $table = $wpdb->prefix.'ebay_stocks_log';
            ...
            if ( $delete_count ) {
                $wpdb->query('DELETE FROM '.$table.' WHERE timestamp < DATE_SUB(NOW(), INTERVAL '.$days_to_keep.' DAY )');
        ...
        public function logStockActivity( $meta_id, $object_id, $meta_key, $meta_value ) {
            if ( !apply_filters( 'wple_enable_stock_logging', true ) ) return;
            ...
            WPLE()->logger->notice( $stock_message );
        }
        ...
        private function insertLogRecord( $data ) {
            ...
            // insert into db
            $wpdb->insert( $wpdb->prefix.'ebay_stocks_log', $data );
            if ( $wpdb->last_error ) WPLE()->logger->error( 'Error in WPLE_StocksLogger::insertLogRecord(): '.$wpdb->last_error.' - SQL: '.$wpdb->last_query );
