...
class WpLister_Product_MetaBox {
	...
	function meta_box_gtins( $post ) {
        ...
        ###
        # Disabling product matching temporarily due to eBay requiring a valid oAuth token for this #49329
        # https://developer.ebay.com/DevZone/shopping/docs/CallRef/FindProducts.html
        # @todo Enable after switching to oAuth tokens
        ##
	...
	function showItemSpecifics( $post ) {
		...
		if ( ! $wpl_default_ebay_category_id ) {
            ...
            // load the default category from the Settings > Categories page #48440
            if ( !$wpl_default_ebay_category_id ) {
                $wpl_default_ebay_category_id   = $wpl_account_id ? WPLE()->accounts[ $wpl_account_id ]->default_ebay_category_id : get_option('wplister_default_ebay_category_id');
            }
