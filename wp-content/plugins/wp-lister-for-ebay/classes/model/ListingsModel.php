...
class ListingsModel extends WPL_Model {
	...
	function reviseInventoryStatus( $id, $session, $cart_item = false, $force = false )
	{
		...
		if ( ! $useFixedPriceItem ) {
			...
			wple_show_message( 'Revising the inventory status (price and quantity) is only possible for fixed price items. This is item is not a fixed price item, so the entire listing will be revised on eBay.', 'warn', ['persistent' => true] );
		...
		if ( $isVariableProduct ) {
            ...
            if ( $variations_mode == 'flat' ) {
				...
				wple_show_message( 'Revising only price and quantity is not possible for flattened variations, so the entire listing will be revised on eBay.', 'warn', ['persistent' => true] );
				return $this->reviseItem( $id, $session, true );
            }
            ...
            if ( $result && ! $result->success ) {
				...
				wple_show_message( 'New variations have been added to this product. Revising only price and quantity is not possible when adding new variations, so the entire listing will be revised on eBay.', 'warn', ['persistent' => true] );
				return $this->reviseItem( $id, $session, true );
            }
            ...
            if ( sizeof( $variations ) == 0 ) {
				...
				wple_show_message( 'No variations have been modified, the revise request will be skipped.', 'info', ['persistent' => true] );
	...
	public function updateEndedListings( $session ) {
		...
		foreach ($items as $item) {
			...
			if ( is_object( $item_details = self::decodeObject( $item['details'] ) ) ) {
				$actual_listing_duration = $item_details->ListingDuration;
	...
	public function prepareProductForListing( $post_id, $profile_id = false, $post_content = false, $post_title = false, $parent_id = false, $variation_attributes = null ) {
		...
		$data['variations']    = ($variation_attributes) ? serialize( $variation_attributes ) : null;
	...
	static function applyProfilePrice( $product_price, $profile_price ) {
		...
		$price = apply_filters( 'wple_ebay_price', $price );
		return $price;
	}
	...
	public function applyProfileToItem( $profile, $item, $update_title = true ) {
		...
		if ( ProductWrapper::isSingleVariation( $post_id ) ) {
    	    ...
    	    foreach ($variations as $var) {
    	    	...
    	    	if ( $match ) {
    	    	    WPLE()->logger->info( 'match found for '. $item['variations'] .': '. serialize( $var['variation_attributes'] ) );
		...
		if ( $update_title ) {
			...
			$data['auction_title'] = apply_filters_deprecated( 'wplister_apply_profile_auction_title', array($data['auction_title'], $item, $profile), '2.8.4', 'wple_apply_profile_auction_title' );
			$data['auction_title'] = apply_filters( 'wple_apply_profile_auction_title', $data['auction_title'], $item, $profile );
