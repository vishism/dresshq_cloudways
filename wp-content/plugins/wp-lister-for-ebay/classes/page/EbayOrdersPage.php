...
class EbayOrdersPage extends WPL_Page {
	...
	public function onDisplayEbayOrdersPage() {
		...
		if ( $this->requestAction() == 'wple_update_orders' ) {
            ...
            // Download specific orders from eBay #49563
            if ( !empty( $_REQUEST['wple_download_order_numbers'] ) && !empty( $_REQUEST['wple_download_from_account'] ) ) {
                // download specific order IDs
                $order_ids = array_filter(array_map('trim', explode(',', wple_clean($_REQUEST['wple_download_order_numbers']))));
                $account_id = wple_clean($_REQUEST['wple_download_from_account']);

                if ( !empty( $order_ids ) ) {
                    $this->initEC( $account_id );
                    $tm = $this->EC->updateEbayOrders( false, $order_ids );
                    $this->EC->updateListings();
                    $this->EC->closeEbay();

                    // show ebay_order report
                    $msg .= sprintf( __( '%s order(s) found on eBay.', 'wp-lister-for-ebay' ), $tm->count_total ) . '<br>';
		...
		if ( $this->requestAction() == 'wple_bulk_update_orders' ) {
			if ( isset( $_REQUEST['ebay_order'] ) ) {
				...
				$ebay_orders = EbayOrdersModel::geteBayOrderIds( $ebay_order );

				$this->initEC( $account_id );
				$tm = $this->EC->updateEbayOrders( false, $ebay_orders );
				$this->EC->updateListings();
				$this->EC->closeEbay();
