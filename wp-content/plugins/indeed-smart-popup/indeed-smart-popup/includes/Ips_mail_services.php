...
class Ips_mail_services{
	...
	public function indeed_myMailSubscribe( $listId, $e_mail, $first_name = '', $last_name = '' ){
		$userdata = array(
				'firstname' => $first_name,
				'lastname' => $last_name,
		);
		if(function_exists('mymail_subscribe')){
			$return = mymail_subscribe( $e_mail, $userdata, array($listId));
	...
	public function indeed_getMyMailLists(){
		...
		// print_R($lists);die();
		if(isset($lists) && $lists!=FALSE && count($lists)>0){
			foreach($lists as $v){
				if($v->slug && $v->name) $list_arr[$v->slug] = $v->name;
			}
			return $list_arr;
		}else return FALSE;
	}
