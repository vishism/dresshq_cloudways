...
Description: Plugin that generate pop-up windows
Version: 4.0
Author: indeed
...
function ips_header(){
	if(isset($_REQUEST['page']) && $_REQUEST['page']=='ips_admin'){
		...
		if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == 'ips_stats') {
...
function set_ips_cookie(){
	...
	if(!array_key_exists('ips_visitor',$_COOKIE) || !$_COOKIE['ips_visitor']){
		...
		setcookie('ips_visitor', $ips_user_cookie, time() + ( 365*24*3600 ),"/" );
	}
    global $visit_pages_value;
    if(isset($_COOKIE['ips_visit_pages']) && $_COOKIE['ips_visit_pages']!=''){
    	$visit_pages = (int)$_COOKIE['ips_visit_pages'];
    	$visit_pages++;
    	unset($_COOKIE['ips_visit_pages']);
    	setcookie("ips_visit_pages", $visit_pages, time()+(60*60*24),"/");
    	$visit_pages_value = $visit_pages;
    }
}
...
function ips_shortcode($id){
	...
	if(isset($id) && $id!=''){
		...
		$sessionData = ManageSession($meta_arr, $ips_id, $visit_pages_value, $current_country);
