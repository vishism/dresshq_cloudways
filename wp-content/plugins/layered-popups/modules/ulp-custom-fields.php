				...
				if (is_array($field) && array_key_exists('type', $field)) {
					if (array_key_exists($field['type'], $this->field_types)) {
						switch ($field['type']) {
							...
							case 'date':
								$html = '<input class="ulp-input ulp-date ulp-input-field" type="text" name="ulp-custom-field-'.$field_id.'" placeholder="'.esc_html($field['placeholder']).'" value="'.esc_html($field['value']).'" onfocus="jQuery(this).removeClass(\'ulp-input-error\');" data-format="'.esc_html($this->options['customfields_date_format']).'" data-min-type="'.$field['mindatetype'].'" data-min-value="'.$field['mindatevalue'].'" data-max-type="'.$field['maxdatetype'].'" data-max-value="'.$field['maxdatevalue'].'" />'.($ulp->options['fa_enable'] == 'on' && $popup_options['input_icons'] == 'on' ? '<div class="ulp-fa-input-table"><div class="ulp-fa-input-cell"><i class="fa '.esc_html($field['icon']).'"></i></div></div>' : '');
								break;
