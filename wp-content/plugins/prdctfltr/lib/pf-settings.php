	...
	class WC_Settings_Prdctfltr {
		...
		public static function prdctfltr_admin_scripts($hook) {
			...
			if ( isset( $_GET['page'], $_GET['tab'] ) && ($_GET['page'] == 'wc-settings' || $_GET['page'] == 'woocommerce_settings' ) && $_GET['tab'] == 'settings_products_filter' ) {
				wp_register_style( 'prdctfltr-admin', WC_Prdctfltr::$url_path .'lib/css/admin.css', false, '5.0.0' );
				wp_enqueue_style( 'prdctfltr-admin' );
				wp_register_script( 'prdctfltr-settings', WC_Prdctfltr::$url_path . 'lib/js/admin.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), '5.0.0', true );
				...
				$curr_args = array(
					...
					'localization' => array(
						...
						'customization_remove' => __( 'Removed! Please save your preset or the default filter!', 'prdctfltr' ),
	...
	?>
				...
				<?php
					...
					if ( isset($_POST['pfr_taxonomy']) ) {
						...
						for( $i = 0; $i < count( $_POST['pfr_taxonomy'] ); $i++ ) {
							...
							$pf_filters_range['pfr_custom'][$i] = ( isset($_POST['pfr_custom'][$i]) ? $_POST['pfr_custom'][$i] : '' );
						}
									...
									<table cass="form-table">
										<tbody>
											...
											<tr valign="top">
												...
												<td class="forminp forminp-text">
													<?php
														printf( '<input name="pfa_term_customization[%1$s]" id="pfa_term_customization_%1$s" class="pf_term_customization" type="text" value="%2$s" style="width:300px;margin-right:12px;" /></label>', $i, ( isset( $pf_filters_advanced['pfa_term_customization'][$i] ) && $pf_filters_advanced['pfa_term_customization'][$i] !== '' ? $pf_filters_advanced['pfa_term_customization'][$i] : '' ) );
													?>
												...
												<td class="forminp forminp-select">
													<?php
														...
														foreach ( $taxonomies as $k => $v ) {
															...
															echo '<option value="pa_' . $v->attribute_name . '"' . ( $pf_filters_range['pfr_taxonomy'][$q] == 'pa_' . $v->attribute_name ? ' selected="selected"' : '' ) .'>' . $curr_label . '</option>';
														}
														echo '</select>';
													?>
		...
		<?php
			$sections = array(
				...
				'overrides' => array(
					'title' => __( 'Filter Overrides', 'prdctfltr' ),
		...
		<?php
			...
			if ( isset($_GET['section']) && $_GET['section'] == 'analytics' ) {
				...
				$settings = array(
					'section_general_title' => array(
						'name' => __( 'Product Filter General Shop/Product Archive Settings', 'prdctfltr' ),
						'type' => 'title',
					...
					'prdctfltr_enable_action' => array(
						'name' => __( 'Product Filter Init Action', 'prdctfltr' ),
						'type' => 'text',
					...
					'prdctfltr_ajax_class' => array(
						...
						'type' => 'text',
						'desc' => __( 'Enter custom wrapper class if you are using a broken template and the default setting is not working. Default class: .products', 'prdctfltr' ),
					...
					'prdctfltr_ajax_category_class' => array(
						...
						'type' => 'text',
						'desc' => __( 'Enter custom category class if you are using a broken template and the default setting is not working. Default class: .product-category', 'prdctfltr' ),
					...
					'prdctfltr_ajax_product_class' => array(
						...
						'type' => 'text',
						'desc' => __( 'Enter custom products class if you are using a broken template and the default setting is not working. Default class: .type-product', 'prdctfltr' ),
					...
					'prdctfltr_ajax_pagination_class' => array(
						...
						'type' => 'text',
						'desc' => __( 'Enter custom pagination class if you are using a broken template and the default setting is not working. Default class: .woocommerce-pagination', 'prdctfltr' ),
			...
			<?php
				...
				?>
					<h3><?php _e( 'Product Filter Overrides', 'prdctfltr' ); ?></h3>
		...
		public static function prdctfltr_admin_save() {
			...
			$curr_name = isset( $_POST['curr_name'] ) ? $_POST['curr_name'] : '' ;
				...
				<tbody>
					...
					<tr valign="top">
						...
						<td class="forminp forminp-multiselect">
							<?php
								if ( isset( $_POST['pfa_taxonomy'] ) ) {
									$catalog_attrs = get_terms( $_POST['pfa_taxonomy'] );
								}
								$curr_options = '';
								if ( !empty( $catalog_attrs ) && !is_wp_error( $catalog_attrs ) ){
									foreach ( $catalog_attrs as $term ) {
