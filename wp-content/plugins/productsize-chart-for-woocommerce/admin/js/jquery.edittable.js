...
(function ($, window, i) {
	...
	$.fn.editTable = function (options) {
		...
		var s                   = $.extend(
			...
			defaultth           = '<th><a class="addcol icon-button" href="javascript:void(0)">+</a> <a class="delcol icon-button" href="#">-</a></th>',
			colnumber,
			rownumber,
			reset,
			is_validated        = true;
		...
		function buildRow(data, len) {
			...
			return $(
				'<tr/>',
				{
					html: rowcontent + '<td><a class="addrow icon-button" href="javascript:void(0)">+</a> <a class="delrow icon-button" href="#">-</a></td>'
				}
			);
		...
		function exportData() {
			...
			$table.find( 'tbody tr' ).each(
				function () {
					...
					$( this ).find( 'td:not(:last-child)' ).each(
						function (i, v) {
...
jQuery( window ).ready(
	function ($) {
			...
			$( '#chart-table-1' ).editTable();
			  $( '#chart-table-2' ).editTable();
