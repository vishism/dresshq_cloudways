...
printf( '<%1$s id="modal1Title">%2$s</%3$s>', $title, $assets['label'], $title );
...
<img src="' . esc_url($pimg[0]) . '" alt="' . esc_attr( $post->post_title ) . '"
...
if ( $enable_additional_chart == 1 ) :
	...
	if ( $assets['chart-1'] ) :
		...
		if ( $image_c1 ) :
			...
			echo '<div class="chart-1-image ' . esc_attr($position_c1) . '"><img src="' . esc_url($img[0]) . '" alt="' . esc_attr($title_c1) . '" 
